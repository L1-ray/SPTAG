...
| 优先级 | 方向 | 类型 | 为什么优先 / 不优先 | 成功标准 |
|---:|---|---|---|---|
| P0 | 并发 recall 稳定性 | correctness | `st=8` recall 明显低于 `st=1`，会污染所有 QPS 结论 | 同 query 候选/结果在不同并发下稳定 |
| P1 | payload physical page inflation | performance | metadata/code I/O 已压掉，payload pages 是唯一大头 | `PayloadPhysicalBytesRead` / `PagesRead` 下降且 recall 稳定 |
| P2 | coarse recall / miss-case 指标 | observability | 不知道 recall 损失来自 PQ、topR、prune 还是实现错误 | 能解释每类 miss 的来源 |
| P3 | M3 pruning 重做 | quality/performance | 当前 pruning 是 heuristic，t32 已伤 recall | safe/heuristic 分档清楚，truth prune 可观测 |
| P4 | code cache 稳定化 | performance guardrail | code I/O 已清零，不宜先回到高风险 async | `CodePhysicalBytesRead=0`，内存/load 成本可接受 |
| P5 | benchmark 记录规范 | process | 防止再次误判 | raw log / stats / report 来源一致 |
...
SearchThreadNum>1 recall drop：当前最大 correctness 风险
...
如果只按“性能收益可能最大”排序，P1 会排第一；但从工程可靠性看，仍应先完成 P0 复核和 P2 观测，否则 payload 优化和 topR/pruning 调参都可能建立在未复核的历史现象之上。
