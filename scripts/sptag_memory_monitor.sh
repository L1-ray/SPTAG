#!/bin/bash
# SPTAG 内存索引性能测试监控脚本
# 用于监测 indexbuilder 和 indexsearcher 运行期间的 CPU、内存使用情况
# 支持自动识别阶段（BuildTree、BuildGraph、Search）
...
示例:
    # 监控构建过程
    $0 -o build.csv -l build.log -- ./Release/indexbuilder -i data.fvecs -o index_dir -a BKT

    # 监控搜索过程（清除缓存）
    $0 -o search.csv -l search.log -C -- ./Release/indexsearcher -i query.fvecs -x index_dir

输出 CSV 列:
    timestamp          - 时间戳 (ISO 8601)
    elapsed_time       - 经过时间 (秒)
    stage              - 当前阶段 (BuildTree/BuildGraph/Search)
    cpu_percent        - CPU 使用率 (%)
...
阶段识别关键词:
    ...
    BuildGraph - "build RNG graph", "BuildGraph time"
    Search     - "[query]", "[maxcheck]", "[recall]", "搜索测试"
    Finished   - "Output results finish", "Failed to build index", "测试总结"
EOF
    exit 0
}
...
# ============ 阶段识别（内存索引版） ============
# 全局变量：记录已检测到的阶段开始标记
STAGE_BUILD_TREE_STARTED=0
STAGE_BUILD_GRAPH_STARTED=0
...
detect_stage() {
    ...
    if [[ $STAGE_BUILD_TREE_STARTED -eq 0 ]]; then
        if grep -qE "Start to build BKTree|Building BKTree|Start building BKT" "$log_file" 2>/dev/null; then
            STAGE_BUILD_TREE_STARTED=1
        fi
    fi
    if [[ $STAGE_BUILD_GRAPH_STARTED -eq 0 ]]; then
        if grep -qE "build RNG graph|BuildGraph|Building graph" "$log_file" 2>/dev/null; then
            STAGE_BUILD_GRAPH_STARTED=1
        fi
    fi
    if [[ $STAGE_SEARCH_STARTED -eq 0 ]]; then
        if grep -qE "\[query\]|\[maxcheck\]|\[recall\]|搜索测试" "$log_file" 2>/dev/null; then
            STAGE_SEARCH_STARTED=1
        fi
    fi

    # 检测是否已完成
    if echo "$recent_logs" | grep -qE "Output results finish|Failed to build index|Finish building|测试总结"; then
        echo "finished"
        return
    fi
    ...
    if [[ $STAGE_BUILD_TREE_STARTED -eq 1 ]]; then
        current_stage="build_tree"
    fi
    if [[ $STAGE_BUILD_GRAPH_STARTED -eq 1 ]]; then
...
sample() {
    ...
    # 实时显示进度
    printf "\r[%8.1fs] 阶段: %-12s CPU: %5.1f%%  内存: %6.1f MB  磁盘读取: %6.2f MB/s  写入: %6.2f MB/s" \
