#!/usr/bin/env python3
"""
SIFT10M Train Learned Policy with ir=64 (24 features)
"""
...
RESULT_DIR = '/home/ray/code/SPTAG/results/adaptive_budget/sift10m_ir64_retrain'

def extract_features_from_trace():
    """Extract features from head_distance_trace.csv"""
...
def main():
    print("="*60)
    print("SIFT10M Learned Policy Training (ir=64, 24 features)")
    print("="*60)
    ...
    rng = RandomState(42)
    test_indices = rng.choice(len(features_df), size=int(0.2 * len(features_df)), replace=False)
    test_mask = features_df.index.isin(test_indices)
    train_df = features_df[~test_mask].reset_index(drop=True)
