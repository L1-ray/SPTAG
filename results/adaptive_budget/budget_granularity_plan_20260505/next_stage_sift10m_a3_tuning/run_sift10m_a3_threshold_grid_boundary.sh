...
echo "tag,t32,t40,t48,t56,qps,recall10" > "$OUT/grid_raw_boundary.csv"
...
EOINI
  ...
  echo "$tag,$t32,$t40,$t48,$t56,$qps,$recall" >> "$OUT/grid_raw_boundary.csv"
}
...
for g in "${GRID[@]}"; do
  ...
  tag=$(printf "b%02d_t32_%s_t40_%s_t48_%s_t56_%s" "$idx" "$t32" "$t40" "$t48" "$t56" | tr '.' 'p')
...
raw=pd.read_csv(out/'grid_raw_boundary.csv')
...
res=pd.DataFrame(rows).sort_values('delta_qps_vs_b64_pct',ascending=False)
res.to_csv(out/'grid_enriched_boundary.csv',index=False)
cons=res[res['delta_recall_vs_b64']>=-0.0015].sort_values('delta_qps_vs_b64_pct',ascending=False)
cons.to_csv(out/'grid_constrained_boundary.csv',index=False)
print(res[['tag','qps','recall10','delta_qps_vs_b64_pct','delta_recall_vs_b64']])
print('\nfeasible:')
print(cons[['tag','qps','recall10','delta_qps_vs_b64_pct','delta_recall_vs_b64']])
PY
