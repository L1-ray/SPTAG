...
# Grid around current A3 thresholds: 32:0.95,40:0.92,48:0.88,56:0.82
GRID=(
  "0.95 0.92 0.88 0.82"
  "0.95 0.91 0.87 0.81"
  "0.95 0.90 0.86 0.80"
...
echo "tag,t32,t40,t48,t56,qps,recall10" > "$OUT/grid_raw.csv"
...
EOINI
  ...
  echo "$tag,$t32,$t40,$t48,$t56,$qps,$recall" >> "$OUT/grid_raw.csv"
}
...
for g in "${GRID[@]}"; do
  ...
  tag=$(printf "g%02d_t32_%s_t40_%s_t48_%s_t56_%s" "$idx" "$t32" "$t40" "$t48" "$t56" | tr '.' 'p')
  run_one "$tag" "$t32" "$t40" "$t48" "$t56"
done

# Baselines for comparison
# fixed-B64 baseline
cat > "$OUT/baseline_b64.ini" << EOB
[Base]
ValueType=UInt8
DistCalcMethod=L2
IndexAlgoType=BKT
...
GenerateTruth=false
IndexDirectory=/home/ray/data/sift10m/spann_index_u8default_20260430
SearchResult=${OUT}/baseline_b64.bin
HeadIndexFolder=head_index
...
SearchPostingPageLimit=4
EnableDetailedIOStats=true
DetailedIOStatsOutput=${OUT}/baseline_b64.csv
DetailedIOStatsSampleRate=1.0
EnablePageCache=false
EnableInFlightCoalescing=true
EOB
sync && echo 3 | sudo tee /proc/sys/vm/drop_caches > /dev/null
"$BIN" "$OUT/baseline_b64.ini" > "$OUT/baseline_b64.log" 2>&1

# current A3 reference
cat > "$OUT/a3_ref.ini" << EOR
[Base]
ValueType=UInt8
DistCalcMethod=L2
IndexAlgoType=BKT
...
SearchPostingPageLimit=4
EnableDetailedIOStats=true
DetailedIOStatsOutput=${OUT}/a3_ref.csv
DetailedIOStatsSampleRate=1.0
EnablePageCache=false
EnableInFlightCoalescing=true
EnableLearnedBudget=true
...
LearnedBudgetThreshold=0.85
LearnedBudgetCandidates=32,40,48,56
LearnedBudgetThresholds=32:0.95,40:0.92,48:0.88,56:0.82
LearnedBudgetDefault=64
LearnedBudgetMin=32
EOR
sync && echo 3 | sudo tee /proc/sys/vm/drop_caches > /dev/null
"$BIN" "$OUT/a3_ref.ini" > "$OUT/a3_ref.log" 2>&1
...
raw=pd.read_csv(out/'grid_raw.csv')
...
res=res.sort_values(['delta_qps_vs_a3_pct','delta_recall_vs_a3'],ascending=[False,False])
res.to_csv(out/'grid_enriched.csv',index=False)

# constrained view: recall drop vs b64 >= -0.0015
cons=res[res['delta_recall_vs_b64']>=-0.0015].copy()
cons=cons.sort_values('delta_qps_vs_b64_pct',ascending=False)
cons.to_csv(out/'grid_constrained.csv',index=False)

with open(out/'tuning_report.md','w') as f:
    f.write('# SIFT10M A3 Threshold Tuning (Next Stage)\n\n')
    f.write(f'baseline_b64_qps,{b_qps},baseline_b64_recall,{b_rec}\n\n')
    f.write(f'a3_ref_qps,{a_qps},a3_ref_recall,{a_rec}\n\n')
    f.write('## Enriched Grid\n\n```csv\n')
    f.write(res.to_csv(index=False))
    f.write('```\n\n')
    f.write('## Constrained Candidates (delta_recall_vs_b64 >= -0.0015)\n\n```csv\n')
    f.write(cons.to_csv(index=False))
    f.write('```\n')

print('Top-5 by delta_qps_vs_a3_pct:')
print(res[['tag','qps','recall10','delta_qps_vs_a3_pct','delta_recall_vs_a3','delta_qps_vs_b64_pct','delta_recall_vs_b64']].head(5))
print('\nConstrained top by delta_qps_vs_b64_pct:')
print(cons[['tag','qps','recall10','delta_qps_vs_b64_pct','delta_recall_vs_b64']].head(5))
PY
