...
{
template <typename ValueType> class ExtraDynamicSearcher : public IExtraSearcher
{
    struct AppendPair
    {
        std::string BKTID;
        ...
        AppendPair(std::string p_BKTID = "", int p_headID = -1, std::string p_posting = "")
            : BKTID(p_BKTID), headID(p_headID), posting(p_posting)
        {
        }
    ...
    class MergeAsyncJob : public Helper::ThreadPool::Job
    {
      ...
      public:
        MergeAsyncJob(VectorIndex *headIndex, ExtraDynamicSearcher<ValueType> *extraIndex, SizeType headID,
                      bool disableReassign, std::function<void()> p_callback)
            : m_index(headIndex), m_extraIndex(extraIndex), headID(headID), disableReassign(disableReassign),
    ...
    class SplitAsyncJob : public Helper::ThreadPool::Job
    {
      ...
      public:
        SplitAsyncJob(VectorIndex *headIndex, ExtraDynamicSearcher<ValueType> *extraIndex, SizeType headID,
                      bool disableReassign, std::function<void()> p_callback)
            : m_index(headIndex), m_extraIndex(extraIndex), headID(headID), disableReassign(disableReassign),
              m_callback(std::move(p_callback))
        {
    ...
    class ReassignAsyncJob : public Helper::ThreadPool::Job
    {
      ...
      public:
        ReassignAsyncJob(VectorIndex *headIndex, ExtraDynamicSearcher<ValueType> *extraIndex,
            ...
            : m_index(headIndex), m_extraIndex(extraIndex), vectorInfo(std::move(vectorInfo)), HeadPrev(HeadPrev),
              m_callback(std::move(p_callback))
        {
        }
  ...
  public:
    ExtraDynamicSearcher(SPANN::Options &p_opt)
    {
        ...
        p_opt.m_postingPageLimit =
            max(p_opt.m_postingPageLimit,
                static_cast<int>((p_opt.m_postingVectorLimit * m_vectorInfoSize + PageSize - 1) / PageSize));
...
#else
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                         "ExtraDynamicSearcher:SPDK unsupport! Use -DSPDK to enable SPDK when doing cmake.\n");
            return;
#endif
        }
        else if (p_opt.m_storage == Storage::ROCKSDBIO)
        {
#ifdef ROCKSDB
            std::string indexDir =
                (p_opt.m_recovery) ? p_opt.m_persistentBufferPath + FolderSep : p_opt.m_indexDirectory + FolderSep;
            ...
            db.reset(new RocksDBIO((indexDir + p_opt.m_KVFile).c_str(), p_opt.m_useDirectIO, p_opt.m_enableWAL,
                                   p_opt.m_recovery));
#else
            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                         "ExtraDynamicSearcher:RocksDB unsupport! Use -DROCKSDB to enable RocksDB when doing cmake.\n");
            return;
#endif
        }
    ...
    bool IsAssumptionBroken(VectorIndex *p_index, SizeType headID, ValueType *vector, SizeType vid)
    {
        ...
        {
            rec_query.reset((uint8_t *)ALIGN_ALLOC(p_index->m_pQuantizer->ReconstructSize()),
        ...
        for (int i = 0; i < headCandidates.GetResultNum() && replicaCount < m_opt->m_replicaCount; ++i)
        {
            ...
            if (selections[replicaCount].node == headID)
                return false;
            ++replicaCount;
        }
        return true;
    ...
    int QuantifyAssumptionBroken(VectorIndex *p_index, SizeType headID, std::string &postingList, SizeType SplitHead,
                                 std::vector<SizeType> &newHeads, std::set<int> &brokenID, int topK = 0,
                                 float ratio = 1.0)
    {
        ...
        for (int j = 0; j < postVectorNum; j++)
        {
            ...
            float_t dist = p_index->ComputeDistance(reinterpret_cast<ValueType *>(vectorId + m_metaDataSize),
                                                    p_index->GetSample(headID));
            ...
            if (brokenID.find(vid) == brokenID.end() &&
                IsAssumptionBroken(p_index, headID, reinterpret_cast<ValueType *>(vectorId + m_metaDataSize), vid))
            {
                /*
                float_t headDist = p_index->ComputeDistance(headCandidates.GetTarget(), p_index->GetSample(SplitHead));
                float_t newHeadDist_1 = p_index->ComputeDistance(headCandidates.GetTarget(),
                p_index->GetSample(newHeads[0])); float_t newHeadDist_2 =
                p_index->ComputeDistance(headCandidates.GetTarget(), p_index->GetSample(newHeads[1]));

                float_t splitDist = p_index->ComputeDistance(p_index->GetSample(SplitHead), p_index->GetSample(headID));

                float_t headToNewHeadDist_1 = p_index->ComputeDistance(p_index->GetSample(headID),
                p_index->GetSample(newHeads[0])); float_t headToNewHeadDist_2 =
                p_index->ComputeDistance(p_index->GetSample(headID), p_index->GetSample(newHeads[1]));

                SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "broken vid to head distance: %f, to split head distance: %f\n",
                dist, headDist); SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "broken vid to new head 1 distance: %f, to new
                head 2 distance: %f\n", newHeadDist_1, newHeadDist_2); SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "head to
                spilit head distance: %f\n", splitDist); SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "head to new head 1
                distance: %f, to new head 2 distance: %f\n", headToNewHeadDist_1, headToNewHeadDist_2);
                */
                assumptionBrokenNum++;
                brokenID.insert(vid);
            }
        ...
        if (assumptionBrokenNum != 0)
        {
            ...
            // maxDist, avgDist/postVectorNum, distanceSet[distanceSet.size() * 0.5]);
            // SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "assumption broken num: %d\n", assumptionBrokenNum);
            float_t splitDist = p_index->ComputeDistance(p_index->GetSample(SplitHead), p_index->GetSample(headID));

            float_t headToNewHeadDist_1 =
                p_index->ComputeDistance(p_index->GetSample(headID), p_index->GetSample(newHeads[0]));
            float_t headToNewHeadDist_2 =
                p_index->ComputeDistance(p_index->GetSample(headID), p_index->GetSample(newHeads[1]));

            // SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "head to spilt head distance: %f/%d/%.2f\n", splitDist, topK,
            // ratio); SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "head to new head 1 distance: %f, to new head 2 distance:
            // %f\n", headToNewHeadDist_1, headToNewHeadDist_2);
        }
    ...
    int QuantifySplitCaseA(VectorIndex *p_index, std::vector<SizeType> &newHeads,
                           std::vector<std::string> &postingLists, SizeType SplitHead, int split_order,
                           std::set<int> &brokenID)
    {
        int assumptionBrokenNum = 0;
        assumptionBrokenNum +=
            QuantifyAssumptionBroken(p_index, newHeads[0], postingLists[0], SplitHead, newHeads, brokenID);
        assumptionBrokenNum +=
            QuantifyAssumptionBroken(p_index, newHeads[1], postingLists[1], SplitHead, newHeads, brokenID);
    ...
    void QuantifySplitCaseB(ExtraWorkSpace *p_exWorkSpace, VectorIndex *p_index, SizeType headID,
                            std::vector<SizeType> &newHeads, SizeType SplitHead, int split_order,
                            int assumptionBrokenNum_top0, std::set<int> &brokenID)
    {
        ...
        for (i = 0; i < nearbyHeads.GetResultNum(); i++)
        {
            ...
            if (i == topk)
            {
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                             "After Split%d, Top%d nearby posting lists, caseB : %d in %d/%d\n", split_order, i,
                             assumptionBrokenNum, containedHead, vectorNum);
                topk *= 2;
            }
            if (queryResults[i].VID == newHeads[0] || queryResults[i].VID == newHeads[1])
                continue;
            db->Get(queryResults[i].VID, &postingList, MaxTimeout, &(p_exWorkSpace->m_diskRequests));
            vectorNum += postingList.size() / m_vectorInfoSize;
            int tempNum = QuantifyAssumptionBroken(p_index, queryResults[i].VID, postingList, SplitHead, newHeads,
                                                   brokenID, i, queryResults[i].Dist / queryResults[1].Dist);
    ...
    void QuantifySplit(ExtraWorkSpace *p_exWorkSpace, VectorIndex *p_index, SizeType headID,
                       std::vector<std::string> &postingLists, std::vector<SizeType> &newHeads, SizeType SplitHead,
                       int split_order)
    {
        std::set<int> brokenID;
        brokenID.clear();
        // SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Split Quantify: %d, head1:%d, head2:%d\n", split_order, newHeads[0],
        // newHeads[1]);
        int assumptionBrokenNum = QuantifySplitCaseA(p_index, newHeads, postingLists, SplitHead, split_order, brokenID);
        QuantifySplitCaseB(p_exWorkSpace, p_index, headID, newHeads, SplitHead, split_order, assumptionBrokenNum,
                           brokenID);
    }

    bool CheckIsNeedReassign(VectorIndex *p_index, std::vector<SizeType> &newHeads, ValueType *data, SizeType splitHead,
                             float_t headToSplitHeadDist, float_t currentHeadDist, bool isInSplitHead,
                             SizeType currentHead)
    {
        ...
        if (isInSplitHead)
        {
            if (splitHeadDist >= currentHeadDist)
                return false;
        }
        else
        {
            ...
            if (splitHeadDist <= newHeadDist_1 && splitHeadDist <= newHeadDist_2)
                return false;
            if (currentHeadDist <= newHeadDist_1 && currentHeadDist <= newHeadDist_2)
                return false;
        }
        return true;
    }
    ...
    void CalculatePostingDistribution(VectorIndex *p_index)
    {
        ...
        for (int i = 0; i < page + 2; ++i)
        {
            ...
            {
                if (i == 0)
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "0 ~ 512 B: %d, \n", sizeDistribution[0] - deletedHead);
                else if (i == 1)
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "512 B ~ 1 KB: %d, \n", sizeDistribution[1]);
                else
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "1 KB ~ 4 KB: %d, \n", sizeDistribution[2]);
            }
            else
    ...
    // TODO
    ErrorCode RefineIndex(std::shared_ptr<VectorIndex> &p_index, bool p_prereassign,
                          std::vector<SizeType> *p_headmapping, std::vector<SizeType> *p_mapping) override
    {
        ...
        if (!p_prereassign)
        {
            new_postingSizes.Initialize(p_index->GetNumSamples() - p_index->GetNumDeleted(), p_index->m_iDataBlockSize,
                                        p_index->m_iDataCapacity);
            new_checkSums.Initialize(p_index->GetNumSamples() - p_index->GetNumDeleted(), 1, p_index->m_iDataBlockSize,
                                     p_index->m_iDataCapacity);
        }
        ...
        while (!doneReassign)
        {
                ...
                while (true)
                {
                    ...
                    if (index < currentPostingNum)
                    {
                        ...
                        else
                        {
                            ...
                            if (vectorCount <= m_mergeThreshold)
                                mergelist.insert(p_headmapping->at(index));

                            postingList.resize(vectorCount * m_vectorInfoSize);
                            if ((ret = db->Put(p_headmapping->at(index), postingList, MaxTimeout,
                                               &(workSpace.m_diskRequests))) != ErrorCode::Success)
                            {
                                SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                            ...
                            *new_checkSums[p_headmapping->at(index)] =
                            ...
                            if (m_opt->m_consistencyCheck &&
                                (ret = db->Check(p_headmapping->at(index),
                                                 new_postingSizes.GetSize(p_headmapping->at(index)) * m_vectorInfoSize,
                                                 nullptr)) != ErrorCode::Success)
                            {
                                SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "RefineIndex: Check failed after Put %d\n",
            ...
            for (int j = 0; j < m_opt->m_iSSDNumberOfThreads; j++)
            {
            ...
            double elapsedSeconds =
                std::chrono::duration_cast<std::chrono::seconds>(preReassignTimeEnd - preReassignTimeBegin).count();
            ...
            else
            {
                ...
                if ((finalcode = m_versionMap->Load(m_opt->m_indexDirectory + FolderSep + m_opt->m_deleteIDFile,
                    ...
                    ErrorCode::Success)
                    return finalcode;
                if ((finalcode = m_vectorTranslateMap->Load(m_opt->m_indexDirectory + FolderSep + m_opt->m_headIDFile,
                                                            p_index->m_iDataBlockSize, p_index->m_iDataCapacity)) !=
                    ErrorCode::Success)
                    return finalcode;
                if ((finalcode = VectorIndex::LoadIndex(m_opt->m_indexDirectory + FolderSep + m_opt->m_headIndexFolder,
    ...
    ErrorCode Split(ExtraWorkSpace *p_exWorkSpace, VectorIndex *p_index, const SizeType headID, bool reassign = false,
                    bool preReassign = false, bool requirelock = true)
    {
        ...
        Retry:
            ...
            if ((ret = db->Get(headID, &postingList, MaxTimeout, &(p_exWorkSpace->m_diskRequests))) !=
                    ErrorCode::Success ||
                !m_checkSum.ValidateChecksum(postingList.c_str(), (int)(postingList.size()), *m_checkSums[headID]))
            {
                SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
            ...
            elapsedMSeconds =
                std::chrono::duration_cast<std::chrono::microseconds>(splitGetEnd - splitGetBegin).count();
            m_stat.m_getCost += elapsedMSeconds;
            // reinterpret postingList to vectors and IDs
            auto *postingP = reinterpret_cast<uint8_t *>(postingList.data());
            SizeType postVectorNum = (SizeType)(postingList.size() / m_vectorInfoSize);

            // SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "DEBUG: db get Posting %d successfully with length %d real
            // length:%d vectorNum:%d\n", headID, (int)(postingList.size()), m_postingSizes.GetSize(headID),
            // postVectorNum);
            COMMON::Dataset<ValueType> smallSample(postVectorNum, m_opt->m_dim, p_index->m_iDataBlockSize,
                                                   p_index->m_iDataCapacity, (ValueType *)postingP, true, nullptr,
                                                   m_metaDataSize, m_vectorInfoSize);
            // COMMON::Dataset<ValueType> smallSample(0, m_opt->m_dim, p_index->m_iDataBlockSize,
            // p_index->m_iDataCapacity);  // smallSample[i] -> VID std::vector<int> localIndicesInsert(postVectorNum);
            // // smallSample[i] = j <-> localindices[j] = i std::vector<uint8_t>
            // localIndicesInsertVersion(postVectorNum);
            std::vector<int> localIndices;
            localIndices.reserve(postVectorNum);
            uint8_t *vectorId = postingP;
            for (int j = 0; j < postVectorNum; j++, vectorId += m_vectorInfoSize)
            {
                // LOG(Helper::LogLevel::LL_Info, "vector index/total:id: %d/%d:%d\n", j, m_postingSizes[headID].load(),
                ...
                // if (VID >= m_versionMap->Count()) SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "DEBUG: vector ID:%d total
            ...
            if (!preReassign && localIndices.size() < m_postingSizeLimit)
            {
                ...
                // SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "DEBUG: in place or not prereassign & index <
                // m_postingSizeLimit. GC begin...\n");
                ...
                if ((ret = db->Put(headID, postingList, MaxTimeout, &(p_exWorkSpace->m_diskRequests))) !=
                    ErrorCode::Success)
                {
            ...
            SPTAG::COMMON::KmeansArgs<ValueType> args(2, smallSample.C(), (SizeType)localIndices.size(), 1,
                                                      p_index->GetDistCalcMethod(), p_index->m_pQuantizer);
            std::shuffle(localIndices.begin(), localIndices.end(), std::mt19937(std::random_device()()));

            int numClusters = SPTAG::COMMON::KmeansClustering(
                smallSample, localIndices, 0, (SizeType)localIndices.size(), args, 1000, 100.0F, false, nullptr);
            ...
            // int numClusters = ClusteringSPFresh(smallSample, localIndices, 0, localIndices.size(), args, 10, false,
            // m_opt->m_virtualHead);
            if (numClusters <= 1)
            {
                int cut = 1;
                if (m_opt->m_oneClusterCutMax)
                    cut = m_postingSizeLimit;
                ...
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                             "Cluserting Failed (The same vector), Cluster total dist:%f Only Keep %d vectors.\n",
                             totaldist, cut);

                if ((ret = db->Put(headID, newpostingList, MaxTimeout, &(p_exWorkSpace->m_diskRequests))) !=
                    ErrorCode::Success)
                {
            ...
            for (int k = 0; k < 2; k++)
            {
                ...
                for (int j = 0; j < args.counts[k]; j++, ptr += m_vectorInfoSize)
                {
                    ...
                    // Serialize(ptr, localIndicesInsert[localIndices[first + j]],
                    // localIndicesInsertVersion[localIndices[first + j]], smallSample[localIndices[first + j]]);
                }
                if (!theSameHead &&
                    p_index->ComputeDistance(args.centers + k * args._D, p_index->GetSample(headID)) < Epsilon)
                {
                    newHeadsID.push_back(headID);
                    newHeadVID = headID;
                    theSameHead = true;
                    auto splitPutBegin = std::chrono::high_resolution_clock::now();
                    if (!preReassign && (ret = db->Put(newHeadVID, newPostingLists[k], MaxTimeout,
                    ...
                    *m_checkSums[newHeadVID] =
                    ...
                    if (m_opt->m_consistencyCheck &&
                        (ret = db->Check(newHeadVID, m_postingSizes.GetSize(newHeadVID) * m_vectorInfoSize, nullptr)) !=
                            ErrorCode::Success)
                    {
                    ...
                    elapsedMSeconds =
                        std::chrono::duration_cast<std::chrono::microseconds>(splitPutEnd - splitPutBegin).count();
                ...
                else
                {
                    ...
                    if (m_opt->m_excludehead)
                    {
                        ...
                        uint8_t version =
                            *((uint8_t *)(postingP + args.clusterIdx[k] * m_vectorInfoSize + sizeof(SizeType)));
                    ...
                    if (m_postingSizes.AddBatch(1) == ErrorCode::MemoryOverFlow ||
                        m_checkSums.AddBatch(1) == ErrorCode::MemoryOverFlow)
                    {
                    ...
                    *m_checkSums[newHeadVID] =
                    ...
                    elapsedMSeconds =
                        std::chrono::duration_cast<std::chrono::milliseconds>(updateHeadEnd - updateHeadBegin).count();
                    m_stat.m_updateHeadCost += elapsedMSeconds;
                }
                // SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Head id: %d split into : %d, length: %d\n", headID,
                // newHeadVID, args.counts[k]);
                first += args.counts[k];
            }
            if (!theSameHead)
            {
        ...
        if (reassign)
        {
            ...
            elapsedMSeconds =
                std::chrono::duration_cast<std::chrono::milliseconds>(reassignScanEnd - reassignScanBegin).count();
    ...
    {
        {
            ...
            if ((ret = db->Get(headID, &currentPostingList, MaxTimeout, &(p_exWorkSpace->m_diskRequests))) !=
                    ErrorCode::Success ||
                !m_checkSum.ValidateChecksum(currentPostingList.c_str(), (int)(currentPostingList.size()),
                                             *m_checkSums[headID]))
            {
                SPTAGLIB_LOG(
                    Helper::LogLevel::LL_Error,
                    "Fail to get original merge postings: %d, required size:%d, get size:%d, checksum issue:%d\n",
                    headID, (int)(m_postingSizes.GetSize(headID) * m_vectorInfoSize), (int)(currentPostingList.size()),
            ...
            {
                if ((ret = db->Put(headID, mergedPostingList, MaxTimeout, &(p_exWorkSpace->m_diskRequests))) !=
                    ErrorCode::Success)
                {
            ...
            COMMON::QueryResultSet<ValueType> queryResults((ValueType *)(p_index->GetSample(headID)),
                ...
                {
                    {
                        ...
                        if (m_rwLocks.hash_func(queryResult->VID) != m_rwLocks.hash_func(headID))
                            anotherLock.lock();
                        if (!p_index->ContainSample(queryResult->VID))
                            continue;
                        if ((ret = db->Get(queryResult->VID, &nextPostingList, MaxTimeout,
                                           &(p_exWorkSpace->m_diskRequests))) != ErrorCode::Success ||
                            !m_checkSum.ValidateChecksum(nextPostingList.c_str(), (int)(nextPostingList.size()),
                        ...
                        if (currentLength > nextLength)
                        {
                            ...
                            if ((ret = db->Put(headID, mergedPostingList, MaxTimeout,
                                               &(p_exWorkSpace->m_diskRequests))) != ErrorCode::Success)
                            {
                                SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                                             "MergePostings fail to override postings after merge\n");
                                m_mergeLock.unlock();
                                return ret;
                            }
                            ...
                            *m_checkSums[headID] =
                            ...
                            {
                                SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                                             "MergePostings fail to check old posting %d in Merge\n", headID);
                                m_mergeLock.unlock();
                                return ret;
                            }
                        ...
                        {
                            p_index->DeleteIndex(headID);
                            if ((ret = db->Put(queryResult->VID, mergedPostingList, MaxTimeout,
                                               &(p_exWorkSpace->m_diskRequests))) != ErrorCode::Success)
                            {
                                SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                                             "MergePostings fail to override postings after merge\n");
                            ...
                            *m_checkSums[headID] = 0;
                            if (m_opt->m_consistencyCheck &&
                                (ret = db->Check(queryResult->VID,
                                                 m_postingSizes.GetSize(queryResult->VID) * m_vectorInfoSize,
                                                 nullptr)) != ErrorCode::Success)
                            {
                                SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                                             "MergePostings fail to check nearby posting %d in Merge\n",
                                             queryResult->VID);
                                m_mergeLock.unlock();
                                return ret;
                            }
                    ...
                    if (reassign)
                    {
                        ...
                        if (currentLength > nextLength)
                        {
                            ...
                            for (int j = 0; j < nextLength; j++)
                            {
                                ...
                                float origin_dist =
                                    p_index->ComputeDistance(p_index->GetSample(queryResult->VID), vector);
                                float current_dist = p_index->ComputeDistance(p_index->GetSample(headID), vector);
                                if (current_dist > origin_dist)
                                    ReassignAsync(p_index,
                                                  std::make_shared<std::string>((char *)vectorId, m_vectorInfoSize),
                                                  headID);
                            }
            ...
            m_postingSizes.UpdateSize(headID, currentLength);
            *m_checkSums[headID] = m_checkSum.CalcChecksum(mergedPostingList.c_str(), (int)(mergedPostingList.size()));

            if (m_opt->m_consistencyCheck && (ret = db->Check(headID, m_postingSizes.GetSize(headID) * m_vectorInfoSize,
                                                              nullptr)) != ErrorCode::Success)
            {
    ...
    inline void SplitAsync(VectorIndex *p_index, SizeType headID, std::function<void()> p_callback = nullptr)
    {
        ...
        // m_postingSizes.GetSize(headID)); tbb::concurrent_hash_map<SizeType, SizeType>::const_accessor headIDAccessor;
    ...
    inline void ReassignAsync(VectorIndex *p_index, std::shared_ptr<std::string> vectorInfo, SizeType HeadPrev,
                              ...
                              bool theSameHead)
    {
        ...
        for (int i = 0; i < postingLists.size(); i++)
        {
            ...
            for (int j = 0; j < postVectorNum; j++)
            {
                ...
                uint8_t version = *(reinterpret_cast<uint8_t *>(vectorId + sizeof(int)));
                ValueType *vector = reinterpret_cast<ValueType *>(vectorId + m_metaDataSize);
                if (reAssignVectorsTopK.find(vid) == reAssignVectorsTopK.end() && !m_versionMap->Deleted(vid) &&
                    m_versionMap->GetVersion(vid) == version)
                {
                    ...
                    if (CheckIsNeedReassign(p_index, newHeadsID, vector, headID, newHeadsDist[i], dist, true,
                                            newHeadsID[i]))
                    {
                        ReassignAsync(p_index, std::make_shared<std::string>((char *)vectorId, m_vectorInfoSize),
                                      newHeadsID[i]);
        ...
        if (m_opt->m_reassignK > 0)
        {
            ...
            auto elapsedMSeconds =
                std::chrono::duration_cast<std::chrono::microseconds>(reassignScanIOEnd - reassignScanIOBegin).count();
            ...
            for (int i = 0; i < HeadPrevTopK.size(); i++)
            {
                ...
                for (int j = 0; j < postVectorNum; j++)
                {
                    ...
                    // HeadPrevTopK[i], postingLists.size(), HeadPrevTopK.size());
                        ...
                        m_versionMap->GetVersion(vid) == version)
                    {
                        ...
                        if (CheckIsNeedReassign(p_index, newHeadsID, vector, headID, newHeadsDist[i], dist, false,
                                                HeadPrevTopK[i]))
                        {
                            ReassignAsync(p_index, std::make_shared<std::string>((char *)vectorId, m_vectorInfoSize),
                                          HeadPrevTopK[i]);
    ...
    bool RNGSelection(std::vector<Edge> &selections, ValueType *queryVector, VectorIndex *p_index, SizeType p_fullID,
                      int &replicaCount, int checkHeadID = -1)
    {
    ...
    void InitWorkSpace(ExtraWorkSpace *p_exWorkSpace, bool clear = false) override
    {
        if (clear)
        {
            p_exWorkSpace->Clear(
                ...
                (max(m_opt->m_postingPageLimit, m_opt->m_searchPostingPageLimit) + m_opt->m_bufferLength) << PageSizeEx,
                true, m_opt->m_enableDataCompression);
        }
        else
        {
            p_exWorkSpace->Initialize(
                m_opt->m_maxCheck, m_opt->m_hashExp, max(m_opt->m_searchInternalResultNum, m_opt->m_reassignK),
                (max(m_opt->m_postingPageLimit, m_opt->m_searchPostingPageLimit) + m_opt->m_bufferLength) << PageSizeEx,
                true, m_opt->m_enableDataCompression);
            int wid = 0;
            if (m_freeWorkSpaceIds == nullptr || !m_freeWorkSpaceIds->try_pop(wid))
            {
                SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "FreeWorkSpaceIds is not initalized or the workspace number "
                                                         "is not enough! Please increase iothread number.\n");
            ...
            p_exWorkSpace->m_callback = [m_freeWorkSpaceIds = m_freeWorkSpaceIds, wid]() {
                if (m_freeWorkSpaceIds)
                    m_freeWorkSpaceIds->push(wid);
            };
        }
    }

    ErrorCode AsyncAppend(ExtraWorkSpace *p_exWorkSpace, VectorIndex *p_index, SizeType headID, int appendNum,
                          std::string &appendPosting, int reassignThreshold = 0)
    {
        if (m_asyncAppendQueue.size() >= m_opt->m_asyncAppendQueueSize)
        {
            ...
            {
                if ((ret = Append(p_exWorkSpace, p_index, workPair.headID, 1, workPair.posting, reassignThreshold)) !=
                    ErrorCode::Success)
                {
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
    ...
    ErrorCode Append(ExtraWorkSpace *p_exWorkSpace, VectorIndex *p_index, SizeType headID, int appendNum,
                     std::string &appendPosting, int reassignThreshold = 0)
    {
    ...
    checkDeleted:
        if (!p_index->ContainSample(headID))
        {
            for (int i = 0; i < appendNum; i++)
            {
                ...
                // SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Head Miss Do Not To ReAssign: VID: %d, version: %d, current
                // version: %d\n", *(int*)(&appendPosting[idx]), m_versionMap->GetVersion(*(int*)(&appendPosting[idx])),
                // version);
            }
            return ErrorCode::Success;
        }
        double appendIOSeconds = 0;
        {
            ...
            if (m_postingSizes.GetSize(headID) + appendNum > (m_postingSizeLimit + m_bufferSizeLimit))
            {
                SPTAGLIB_LOG(Helper::LogLevel::LL_Warning,
                             "After appending, the number of vectors in %d exceeds the postingsize + buffersize (%d + "
                             "%d)! Do split now...\n",
            ...
            if ((ret =
                     db->Merge(headID, appendPosting, MaxTimeout, &(p_exWorkSpace->m_diskRequests),
            ...
            appendIOSeconds =
                std::chrono::duration_cast<std::chrono::microseconds>(appendIOEnd - appendIOBegin).count();
            *m_checkSums[headID] =
        ...
        if (m_postingSizes.GetSize(headID) > (m_postingSizeLimit + reassignThreshold))
        {
            ...
            // reassignThreshold); GetDBStats(); if (m_postingSizes.GetSize(headID) > 120) {
            //     GetDBStats();
            // }
    ...
    ErrorCode Reassign(ExtraWorkSpace *p_exWorkSpace, VectorIndex *p_index, std::shared_ptr<std::string> vectorInfo,
                       SizeType HeadPrev)
    {
        ...
        std::vector<Edge> selections(static_cast<size_t>(m_opt->m_replicaCount));
        int replicaCount;
        bool isNeedReassign = RNGSelection(selections, (ValueType *)(vectorInfo->c_str() + m_metaDataSize), p_index,
                                           VID, replicaCount, HeadPrev);
        ...
        if (isNeedReassign && m_versionMap->GetVersion(VID) == version)
        {
            ...
            {
                // LOG(Helper::LogLevel::LL_Info, "Reassign: headID :%d, oldVID:%d, newVID:%d, posting length: %d, dist:
        ...
        elapsedMSeconds =
            std::chrono::duration_cast<std::chrono::microseconds>(reassignAppendEnd - reassignAppendBegin).count();
                   ...
                   COMMON::Dataset<std::uint64_t> &p_vectorTranslateMap, std::shared_ptr<VectorIndex> m_index) override
    {
        ...
        else if (m_opt->m_storage == Storage::SPDKIO || m_opt->m_storage == Storage::FILEIO)
        {
            if (fileexists((m_opt->m_indexDirectory + FolderSep + m_opt->m_ssdIndex).c_str()))
            {
                ...
                m_postingSizes.Initialize((SizeType)(totalPostingNum), m_opt->m_datasetRowsInBlock,
                                          m_opt->m_datasetCapacity);
        ...
        if (m_opt->m_recovery && m_opt->m_enableWAL && m_wal)
        {
            ...
            {
                countAssignment++;
                if (countAssignment % 10000 == 0)
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Process %d logs\n", countAssignment);
                ...
                if (assignment.size() == m_vectorInfoSize)
                {
                    ...
                    vectorSet.reset(
                        new BasicVectorSet(ByteArray((std::uint8_t *)ptr + sizeof(SizeType) + sizeof(uint8_t),
                                                     sizeof(ValueType) * 1 * m_opt->m_dim, false),
    ...
    bool ValidatePostings(std::vector<SizeType> &pids, std::vector<Helper::PageBuffer<std::uint8_t>> &postings)
    {
        ...
        {
            if (!m_checkSum.ValidateChecksum((const char *)(postings[i].GetBuffer()), postings[i].GetAvailableSize(),
                                             *m_checkSums[pids[i]]))
            {
                SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
    ...
    bool ValidatePostings(std::vector<SizeType> &pids, std::vector<std::string> &postings)
    {
        ...
        for (int i = 0; i < pids.size(); i++)
        {
            if (!m_checkSum.ValidateChecksum(postings[i].c_str(), postings[i].size(), *m_checkSums[pids[i]]))
            {
                SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                             "ValidatePostings fail: posting id:%d, required size:%d, buffer size:%d, checksum:%d\n",
                             pids[i], (int)(m_postingSizes.GetSize(pids[i]) * m_vectorInfoSize),
    ...
    virtual ErrorCode SearchIndex(ExtraWorkSpace *p_exWorkSpace, QueryResult &p_queryResults,
                                  ...
                                  std::map<int, std::set<int>> *found) override
    {
        if (p_stats)
            p_stats->m_exSetUpLatency = 0;
        ...
        for (uint32_t pi = 0; pi < postingListCount; ++pi)
        {
            ...
            // SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "DEBUG: postingList %d size:%d m_vectorInfoSize:%d
    ...
    virtual ErrorCode SearchIndexWithoutParsing(ExtraWorkSpace *p_exWorkSpace)
    {
        ...
        if (ret == ErrorCode::Success && !ValidatePostings(p_exWorkSpace->m_postingIDs, p_exWorkSpace->m_pageBuffers))
        {
            return ErrorCode::DiskIOFail;
        }
        return ret;
                                          ...
                                          const VectorIndex *p_spann)
    {
        ...
        while (!foundResult && p_exWorkSpace->m_pi < p_exWorkSpace->m_postingIDs.size())
        {
            ...
            while (p_exWorkSpace->m_offset < vectorNum)
            {
                ...
                int vectorID = *(reinterpret_cast<int *>(vectorInfo));
                if (vectorID >= m_versionMap->Count())
                ...
                if (p_exWorkSpace->m_filterFunc != nullptr &&
                    !p_exWorkSpace->m_filterFunc(p_spann->GetMetadata(vectorID)))
                    continue;
    ...
    bool BuildIndex(std::shared_ptr<Helper::VectorSetReader> &p_reader, std::shared_ptr<VectorIndex> p_headIndex,
                    ...
                    COMMON::Dataset<std::uint64_t> &p_vectorTranslateMap, SizeType upperBound = -1) override
    {
        m_versionMap = &p_versionMap;
        m_vectorTranslateMap = &p_vectorTranslateMap;
        m_opt = &p_opt;
        ...
        }
        {
            ...
            for (int i = 0; i < m_opt->m_batches; i++)
            {
                ...
                p_headIndex->ApproximateRNG(fullVectors, emptySet, candidateNum, selections.m_selections.data(),
                                            m_opt->m_replicaCount, numThreads, m_opt->m_gpuSSDNumTrees,
                                            m_opt->m_gpuSSDLeafSize, m_opt->m_rngFactor, m_opt->m_numGPUs);
                ...
                for (SizeType j = start; j < end; j++)
                {
                    ...
                    {
                        for (int resNum = 0;
                             resNum < m_opt->m_replicaCount && selections[vecOffset + resNum].node != INT_MAX; resNum++)
                        {
                ...
                mythreads.emplace_back([&, tid]() {
                    ...
                    while (true)
                    {
                        ...
                        if (i < postingListSize.size())
                        {
                            ...
                            for (size_t dropID = relaxLimit; dropID < postingListSize[i]; ++dropID)
                            {
        ...
        if (ErrorCode::Success != WriteDownAllPostingToDB(selections, fullVectors))
            return false;
        ...
        if (m_opt->m_update && !m_opt->m_allowZeroReplica && zeroReplicaSet.size() > 0)
        {
            ...
            {
                std::shared_ptr<VectorSet> vectorSet(new BasicVectorSet(
                    ByteArray((std::uint8_t *)fullVectors->GetVector(it), sizeof(ValueType) * m_opt->m_dim, false),
                    GetEnumValueType<ValueType>(), m_opt->m_dim, 1));
            ...
            if (p_headIndex->SaveIndex(m_opt->m_indexDirectory + FolderSep + m_opt->m_headIndexFolder) !=
                ErrorCode::Success)
            {
            ...
            if (m_vectorTranslateMap->Save(m_opt->m_indexDirectory + FolderSep + m_opt->m_headIDFile) !=
                ErrorCode::Success)
            {
            ...
            while (true)
            {
                ...
                if (index < m_postingSizes.GetPostingNum())
                {
                    ...
                    *m_checkSums[index] = m_checkSum.CalcChecksum(postinglist.c_str(), (int)(postinglist.size()));
                    if (m_opt->m_consistencyCheck &&
                        (tmp = db->Check(index, m_postingSizes.GetSize(index) * m_vectorInfoSize, nullptr)) !=
                            ErrorCode::Success)
                    {
                       ...
                       std::shared_ptr<VectorIndex> p_index, SizeType begin) override
    {
        ...
        for (int v = 0; v < p_vectorSet->Count(); v++)
        {
            ...
            for (int i = 0; i < replicaCount; i++)
            {
                ...
                {
                    if ((ret = AsyncAppend(p_exWorkSpace, p_index.get(), selections[i].node, 1, appendPosting)) !=
                        ErrorCode::Success)
                        return ret;
                }
                else
                          ...
                          std::shared_ptr<VectorIndex> p_index, int testNum = 64, SizeType VID = -1) override
    {
        ...
        for (int i = 0; i < queryResults.GetResultNum(); ++i)
        {
            if (db->Get(queryResults.GetResult(i)->VID, &postingList, MaxTimeout, &(p_exWorkSpace->m_diskRequests)) !=
                    ErrorCode::Success ||
                !m_checkSum.ValidateChecksum(postingList.c_str(), (int)(postingList.size()),
                                             *m_checkSums[queryResults.GetResult(i)->VID]))
            {
                continue;
            }
            ...
            for (int j = 0; j < vectorNum; j++)
            {
                ...
                if (VID != -1 && VID == vectorID)
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Find %d in %dth posting\n", VID, i);
                auto distance2leaf =
                    p_index->ComputeDistance(queryResults.GetQuantizedTarget(), vectorInfo + m_metaDataSize);
    ...
    void GetDBStats() override
    {
        db->GetStat();
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                     "remain splitJobs: %d, reassignJobs: %d, running split: %d, running reassign: %d\n",
                              ...
                              bool write = false) override
    {
        ...
        if (write)
        {
            ...
            *m_checkSums[pid] = m_checkSum.CalcChecksum(posting.c_str(), (int)(posting.size()));
            if (m_opt->m_consistencyCheck &&
                (ret = db->Check(pid, m_postingSizes.GetSize(pid) * m_vectorInfoSize, nullptr)) != ErrorCode::Success)
            {
    ...
    ErrorCode GetPostingDebug(ExtraWorkSpace *p_exWorkSpace, std::shared_ptr<VectorIndex> p_index, SizeType vid,
